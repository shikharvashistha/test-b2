...
latexmath:[$t_\{i_1,i_2,\dots,i_p}$] of a latexmath:[$p$]-order latexmath:[$(n_1 \times n_2 \times \cdots
\times n_p)$]-dimensional tensor latexmath:[$T$] is mapped to latexmath:[$j$]-th element of a
one-dimensional container where latexmath:[$j = \sum_\{r=1}^p i_r \cdot w_r$] with
latexmath:[$1 \leq i_r \leq n_r $] for latexmath:[$1 \leq r \leq p$]. For the first-order
orientation latexmath:[$w_1 = 1$] and latexmath:[$w_k = n_\{k-1} \cdot w_\{k-1}$] for latexmath:[$k > 1$].
For last-order orientation latexmath:[$w_p = 1$] and latexmath:[$ w_k = n_\{k+1} \cdot
w_\{k+1}$] for latexmath:[$k < p$].
...
|`template<class derived_type>  using tensor_expression_type = detail::tensor_expression<self_type,derived_type>` |Type of link:tensor/tensor_expression.adoc#tensor_expression[tensor_expression] where `self_type` is the `tensor` type.

|`template<class derived_type>  using matrix_expression_type = matrix_expression<derived_type>` |Type of link:matrix_expression.adoc#matrix_expression[matrix_expression].

|`template<class derived_type>  using vector_expression_type = vector_expression<derived_type>` |Type of link:vector_expression.adoc#vector_expression[vector_expression].
|===
